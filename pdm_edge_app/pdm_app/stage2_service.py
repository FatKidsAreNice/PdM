...
class Stage2Service:
    ...
    def run(self, df: pd.DataFrame) -> Stage2Result:
        ...
        feature_columns.extend(
            [column for column in self.OPTIONAL_FEATURE_COLUMNS if column in working_df.columns]
        )
        ...
        if clean_df.empty:
            raise ValueError(
                "Stage 2 kann nicht berechnet werden, da keine vollständigen Feature-Zeilen vorliegen."
            )
        ...
        threshold = float(
            clean_df["multivariate_score"].quantile(self._config.analysis.stage2.distance_quantile)
        )
        ...
        clean_df["stage2_message"] = np.select(
            condlist=[clean_df["stage2_persistent_warning"], clean_df["stage2_warning_now"]],
            choicelist=[
                "Persistente multivariate Abweichung",
                "Multivariate Einzelwarnung",
            ],
            default="Normalbereich",
        )

        stage2_columns = clean_df[
            [
                "__row_id",
                "pc1",
                "pc2",
        ...
        alerts = working_df.loc[
            ...
            ["timestamp", "stage2_severity", "stage2_message", "multivariate_score", "quality_flag"],
        ].copy()

        alerts = alerts.rename(
            columns={
                "stage2_severity": "severity",
                "stage2_message": "message",
            }
        )

        alerts["stage"] = "Stage 2"
        alerts = alerts[["timestamp", "stage", "severity", "message", "multivariate_score", "quality_flag"]]

        self._logger.info(
            "Stage 2 abgeschlossen: %s multivariate Warnungen/Anomalien",
            len(alerts),
        )
